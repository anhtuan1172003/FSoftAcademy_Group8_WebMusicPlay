...
export default function SongDetail() {
    ...
    const handleFeedbackSubmit = async (e) => {
        ...
        try {
            const response = await fetch('https://dsqkll-8090.csb.app/feedback', {
                method: 'POST',
                headers: {
    ...
    const fetchData = useCallback(async () => {
        try {
            const [artistsRes, albumsRes, songRes, allSongsRes, likesRes] = await Promise.all([
                ...
                fetch(`https://dsqkll-8090.csb.app/listsongs/${sID}`),
                fetch(`https://dsqkll-8090.csb.app/listsongs`),
                user ? fetch(`https://dsqkll-8090.csb.app/like?userid=${user.id}`) : Promise.resolve({ json: () => [] })
            ]);
            ...
            const topSongs = allSongsData.sort((a, b) => b.plays - a.plays).slice(0, 10);
